...
| `0x0003` | `REG_SLEWING_ONLINE_CODE` | 回转 | 设备在线码，`1` 表示通信正常，`0` 表示异常 |
...
| `0x0009` | `REG_LUFFING_ONLINE_CODE` | 变幅 | 在线码，`1` 表示最近一次通信正常，`0` 表示异常 |
| `0x000A` | `REG_LUFFING_TRUN_CNT` | 变幅 | 圈数 |
| `0x000B` | `REG_LUFFING_SINAGLE_VAL` | 变幅 | 单圈值 |
| `0x000C` | `REG_HOIST_TIMESTAMP_H` | 吊钩 | 上一次成功更新时间戳高 16 位 |
| `0x000D` | `REG_HOIST_TIMESTAMP_L` | 吊钩 | 上一次成功更新时间戳低 16 位 |
| `0x000E` | `REG_HOIST_ERROR_CODE` | 吊钩 | RTU通信错误码，`0` 表示正常 |
| `0x000F` | `REG_HOIST_ONLINE_CODE` | 吊钩 | 在线码，`1` 表示最近一次通信正常，`0` 表示异常 |
| `0x0010` | `REG_HOIST_TRUN_CNT` | 吊钩 | 圈数 |
| `0x0011` | `REG_HOIST_SINAGLE_VAL` | 吊钩 | 单圈值 |
...
| 吊钩 `HOIST` | `[12]` 到 `[17]` | `0x000C` 到 `0x0011` |
...
最小读取适合只关心“是否在线、圈数、单圈值”的场景。

每个编码器读取 3 个连续寄存器：

| 机构 | Function Code | Start Address | Quantity | 返回顺序 |
| --- | --- | --- | --- | --- |
| 回转 `SLEWING` | `04` | `0x0003` | `3` | 在线码、圈数、单圈值 |
| 变幅 `LUFFING` | `04` | `0x0009` | `3` | 在线码、圈数、单圈值 |
| 吊钩 `HOIST` | `04` | `0x000F` | `3` | 在线码、圈数、单圈值 |

最小读取不包含错误码和时间戳。如果 `ONLINE_CODE=0`，只能知道该编码器最近一次通信异常；如果需要进一步判断异常原因，请使用完整读取方式读取 `ERROR_CODE`。
...
```c
online = (online_code == 1) && (error_code == 0);
```
...
| 字段 | 异常时含义 |
| --- | --- |
| `ONLINE_CODE=0` | 当前通信状态异常 |
| `ERROR_CODE!=0` | 当前通信错误原因 |
| `TRUN_CNT` | 上一次成功更新的圈数 |
| `SINAGLE_VAL` | 上一次成功更新的单圈值 |
| `TIMESTAMP_H/L` | 上一次成功更新时间 |

上位机不能仅凭圈数或单圈值是否非 0 判断在线状态，必须使用 `ONLINE_CODE`，完整读取时还应同时检查 `ERROR_CODE`。
...
1. `ERROR_CODE=0` 时，`ONLINE_CODE` 应为 `1`。
2. `ERROR_CODE!=0` 时，`ONLINE_CODE` 应为 `0`。
...
```text
0000 7D00 0000 0001 02FC 00AD
0000 7D02 0000 0001 0A76 03F3
0000 7D04 0074 0000 1693 0000
```

解析结果：

| 机构 | 时间戳 | 错误码 | 在线码 | 圈数 | 单圈值 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| 回转 | `0x00007D00 = 32000 ms` | `0` | `1` | `0x02FC` | `0x00AD` | 正常 |
| 变幅 | `0x00007D02 = 32002 ms` | `0` | `1` | `0x0A76` | `0x03F3` | 正常 |
| 吊钩 | `0x00007D04 = 32004 ms` | `116` | `0` | `0x1693` | `0x0000` | 通信超时，数据为上一次成功值 |
...
3. 每 6 个寄存器解析一个编码器。
4. 使用 `ONLINE_CODE` 和 `ERROR_CODE` 判断状态。
5. 如果状态正常，使用圈数和单圈值。
6. 如果状态异常，显示异常状态。
...
2. 每组返回顺序都是：在线码、圈数、单圈值。
3. 使用 `ONLINE_CODE` 判断状态。
4. 如果 `ONLINE_CODE=0`，显示通信异常；如需错误原因，再执行完整读取。
...
| 圈数/单圈值有值但显示异常 | 正常，异常时会保留上一次成功数据，应以 `ONLINE_CODE` 判断状态 |
| 时间戳不变化 | 表示该编码器没有新的成功更新，检查 `ONLINE_CODE` 和 `ERROR_CODE` |
