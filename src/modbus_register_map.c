...
#define MODBUS_DEVICE_CAPABILITY_FLAGS \
	((IS_ENABLED(CONFIG_ENABLE_READ_SLEWING_ENCODER_THREAD) ? DEVICE_CAPABILITY_SLEWING_ENCODER : 0U) | \
	 (IS_ENABLED(CONFIG_ENABLE_READ_LUFFING_ENCODER_THREAD) ? DEVICE_CAPABILITY_LUFFING_ENCODER : 0U) | \
	 (IS_ENABLED(CONFIG_ENABLE_READ_HOISTING_ENCODER_THREAD) ? DEVICE_CAPABILITY_HOISTING_ENCODER : 0U) | \
	 (IS_ENABLED(CONFIG_ENABLE_READ_ANEMOMETER_THREAD) ? DEVICE_CAPABILITY_ANEMOMETER : 0U) | \
	 (IS_ENABLED(CONFIG_ENABLE_READ_LOAD_SENSOR_THREAD) ? DEVICE_CAPABILITY_LOAD_SENSOR : 0U) | \
	 (IS_ENABLED(CONFIG_ENABLE_LUFFING_IMU_APP) ? DEVICE_CAPABILITY_LUFFING_IMU : 0U) | \
